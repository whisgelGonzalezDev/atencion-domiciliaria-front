...
export function NewPatientModal({ open, onClose, onCreated }: NewPatientModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const patient = await patientsApi.create({
        ...
        ...(historyShort.trim() ? { historyShort: historyShort.trim() } : {}),
      })
  ...
  const handleClose = () => {
    setName(''); setAge(''); setPhone(''); setBlood(''); setHistoryShort('')
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md flex flex-col rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-lg overflow-hidden animate-fade-in">
        <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 px-5 py-4">
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Nuevo paciente</h2>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <textarea value={historyShort} onChange={e => setHistoryShort(e.target.value)} placeholder="Antecedentes relevantes, alergias..." rows={2} className={[inputCls(false), 'resize-none'].join(' ')} />
          </div>
